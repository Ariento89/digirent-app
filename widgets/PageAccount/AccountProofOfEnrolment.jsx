...
const AccountProofOfEnrolment = () => {
  // STATES
  const [isSubmitting, setIsSubmitting] = useState(false);

  // CUSTOM HOOKS
  const { addToast } = useToasts();
  const { me, uploadProofOfEnrolment, status, errors } = useMe();
  ...
  const onError = () => {
    addToast('An error occurred while uploading Proof of Enrolment.', toastTypes.ERROR);
  };

  const onSubmit = (data) => {
    uploadProofOfEnrolment(data, { onSuccess, onError });
  };
...
export default AccountProofOfEnrolment;
